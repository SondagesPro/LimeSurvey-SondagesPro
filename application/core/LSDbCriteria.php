...
class LSDbCriteria extends CDbCriteria
{
    ...
    public function addSafeStringSearchCondition($oModel,$condition)
    {
        ...
        if (!is_string($condition) || !preg_match('/^([a-zA-Z0-9_]+)\s*(<=|>=|<>|=|<|>)\s*(.*)$/', $condition, $matches)) {
            throw new CHttpException(400, 'Invalid expression for condition');
        }
    ...
    public function addSafeStructuredSearchCondition($oModel, $aConditions = [])
    {
        ...
        foreach ($aConditions as $columnName => $valueOrTuple) {
            if (!array_key_exists($columnName, $columnsNames)) {
                throw new CHttpException(400, 'Invalid column name: ' . $columnName);
            }
            if (is_array($valueOrTuple)) {
                if (count($valueOrTuple) < 2) {
                    throw new CHttpException(400, 'Invalid number of element for ' . $columnName);
                }
                ...
                if (!is_string($operator)) {
                    throw new CHttpException(400, 'Illegal operator for column ' . $columnName);
                }
                if (!in_array($operator, $allowedOperators, true)) {
                    throw new CHttpException(400, 'Illegal operator: ' . $operator . ' for column ' . $columnName);
                }
                switch ($operator) {
                    case 'LIKE':
                        /** @var scalar */
                        ...
                        if (!is_scalar($value)) {
                            throw new CHttpException(400, 'LIKE operator requires a string value for column ' . $columnName);
                        }
                    ...
                    case 'IN':
                        ...
                        foreach ($values as $v) {
                            if (!is_scalar($v) && !is_null($v)) {
                                throw new CHttpException(400, 'IN operator requires scalar values for column ' . $columnName);
                            }
                        }
                    ...
                    default:
                        ...
                        $value = $valueOrTuple[1];
                        if (!is_scalar($value)) {
                            throw new CHttpException(400, 'Comparison operators require a scalar value for column ' . $columnName);
                        }
                ...
                throw new CHttpException(400, 'Invalid value type for column ' . $columnName);
            }
        }
    }
}
